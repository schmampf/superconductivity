...
# for zoom in np.arange(0.5,3.55, 0.1):

# load coordinates to render
coord = np.loadtxt(r"config_5.txt", delimiter=",")  # config 3 single-atom
# 4 broken; 5 single-atom stretched; 6,7,8,9 double, 12 dimer

z_offset = -50
camera = Camera('location', [60 * zoom, 15, 25 * zoom + z_offset],
...
plane = Plane([0, 1, 0], 1,
              Texture(
                  Pigment(
                      'color', [105/255, 218/255, 1]  # 'OrangeRed'
                  ),
                  Finish(
                      'phong', 0.8,
                      'ambient', 0.2,
...
background = Background("color", [.8, .8, .8])
...
for z in np.arange(6, 110):
    for x in np.arange(-z, z, 1):
        for y in np.arange(-z, z, 1):
            ...
            if abs(d) / (abs(znew ** flattening) + 4) < angle and (abs(d) + 40) / (abs(znew ** flattening) + 4) > angle:
...
filename = 'nu_configuration.png'
full_name = "%03.3f_%s" % (zoom, filename)

scene.render('configuration_ultra.png', width=12000, height=8000)
