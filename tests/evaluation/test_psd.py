...
def _make_psd_trace(value: float) -> psd.PSDTrace:
    return {
        ...
        "I_psd_nA2_per_Hz": np.asarray([value], dtype=np.float64),
        "V_psd_mV2_per_Hz": np.asarray([2.0 * value], dtype=np.float64),
        "nu_Hz": 1.0,
        "nyquist_Hz": 0.5,
    }
...
def test_psd_analysis_returns_collection() -> None:
    ...
    assert np.allclose(out.nyquist_Hz, np.asarray([0.5, 0.5]))
