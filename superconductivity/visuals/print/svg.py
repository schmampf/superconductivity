...
Notes
-----
All physical sizes are specified in millimeters and are converted to
Matplotlib figure sizes internally.
"""
...
def save_axis(
    ...
    The axis is drawn in physical units (millimeters) using a Matplotlib
    figure and exported to ``<dataset>/svg/<title>.svg`` and
    ``<dataset>/png/<title>.png``.

    The arrow represents the data interval ``[min(values), max(values)]``
    mapped to ``arrowlength_mm``. The arrow starts at ``bleed_left_mm`` and
    the resulting canvas is extended by ``bleed_left_mm`` and
    ``bleed_right_mm``.
    ...
    ticks
        ...
        generated using Matplotlib's tick locator with at most ``nticks``
        ticks.
    ticklabels
        Optional explicit tick label strings corresponding to ``ticks``.
    tickformat
        Format string used for numeric tick labels when ``ticklabels`` is
        None.
    nticks
    ...
    if ticks is None:
        ...
        # If the locator returned too many ticks, downsample to at most
        # nticks.
        if ticks_arr.size > int(nticks):
            idx = (
                np.linspace(0, ticks_arr.size - 1, int(nticks))
                .round()
                .astype(int)
            )
    ...
    out_svg = Path(dataset, "svg", f"{title}.svg")
    out_png = Path(dataset, "png", f"{title}.png")
