...
def get_ivf(
    ...
    """Compute one-sided FFTs of ``I(t)`` and ``V(t)``.
    ...
    detrend : bool, default=True
        If ``True``, subtract the mean from ``I_nA`` and ``V_mV`` before FFT.
    ...
    -------
    tuple[np.ndarray, np.ndarray, np.ndarray]
        ``(I_fft_nA, V_fft_mV, f_Hz)`` where:
        - ``I_fft_nA`` is the complex one-sided FFT of current.
        - ``V_fft_mV`` is the complex one-sided FFT of voltage.
    ...
    return (
        np.asarray(I_fft_nA, dtype=np.complex128),
        np.asarray(V_fft_mV, dtype=np.complex128),
