...
def get_I_rstj_nA(
    ...
    dVdI_lut_MOhm = np.gradient(V_iqp_lut_mV, I_lut_nA).astype(np.float32)
    ...
    V_rstj_mV = np.asarray(
        simulate_rstj_with_pat_vac_batch(
            ...
            dVdI_lut_MOhm=jnp.asarray(dVdI_lut_MOhm, dtype=JF32),
    ...
    for i in range(A_arr_mV.size):
        I_bias_nA_over, V_rstj_mV_over = oversample(
            x=I_bias_nA,
            y=V_rstj_mV[i],
        )
...
def simulate_rstj_with_pat_vac_batch(
    ...
    def step(n_i, carry):
        ...
        dVdI_MOhm = lookup_linear_uniform_clamped(
            x0_mV=I_lut0_nA,
            inv_dv_per_mV=inv_dI_per_nA,
            y_grid=dVdI_lut_MOhm,
            x_q_mV=I_for_qp0_nA,
        )
        dVdI_MOhm = jnp.maximum(jnp.abs(dVdI_MOhm), jnp.asarray(JF32EPS, dtype=JF32))

        sigma_nA = jnp.sqrt(sigma_mV2 / dVdI_MOhm)
