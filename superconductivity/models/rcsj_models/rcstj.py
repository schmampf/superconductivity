...
def get_I_rcstj_nA(
    ...
    """
    RCSTJ model (RCSJ + thermal current noise).

    Thermal noise drives phase diffusion through the RCSJ dynamics.
    If `A_mV` is scalar, returns shape `(V_mV.size,)`.
    If `A_mV` is sequence, returns shape `(len(A_mV), V_mV.size)`.
    """
    if C_pF <= 0:
        raise ValueError("C_pF must be > 0 for RCSTJ simulation.")
    if n_periods <= 0:
        raise ValueError("n_periods must be > 0.")
    if not (0.0 <= burn_fraction <= 1.0):
        raise ValueError("burn_fraction must be in [0, 1].")
    if G_min_muS <= 0:
        raise ValueError("G_min_muS must be > 0.")
    ...
    # dI/dV in nA/mV = uS on the same uniform V grid.
    dIdV_lut_muS = np.gradient(I_lut_nA, V_lut_mV).astype(np.float32)
    ...
    V_rcstj_mV = np.asarray(
        simulate_rcstj_with_pat_vac_batch(
            ...
            dIdV_lut_muS=jnp.asarray(dIdV_lut_muS, dtype=JF32),
            G_min_muS=jnp.asarray(G_min_muS, dtype=JF32),
    ...
    if A_is_scalar:
        return I_rcstj_all_nA[0]
    return I_rcstj_all_nA


@functools.partial(jax.jit, static_argnames=("Nt",))
def simulate_rcstj_with_pat_vac_batch(
    I_nA: jnp.ndarray,
    A_mV: jnp.ndarray,
    I_sw_nA: jnp.ndarray,
    C_pF: jnp.ndarray,
    ...
    dIdV_lut_muS: jnp.ndarray,
    G_min_muS: jnp.ndarray,
    nu_GHz: jnp.ndarray,
    dt_ps: jnp.ndarray,
    Nt: int,
    ...
    """
    Batched RCSTJ simulation with thermal current noise.
    ...
    def step(n_i, carry):
        ...
        dIdV_muS = lookup_linear_uniform_clamped(
            x0_mV=V_lut0_mV,
            inv_dv_per_mV=inv_dv_per_mV,
            y_grid=dIdV_lut_muS,
            x_q_mV=V_tot_mV,
        )
        G_eff_muS = jnp.maximum(jnp.abs(dIdV_muS), G_min_muS)
        dVdI_eff_MOhm = jnp.asarray(1.0, dtype=JF32) / G_eff_muS
        sigma_nA = jnp.sqrt(sigma_mV2 / dVdI_eff_MOhm)
        ...
        I_cap_nA = I_row - I_sc_nA - I_qp_nA + I_noise_nA
    ...
    carry0 = (phi0, V0_mV, Vsum0_mV, key0)

    _, _, V_sum_mV, _ = jax.lax.fori_loop(
        0,
        Nt,
        step,
        carry0,
