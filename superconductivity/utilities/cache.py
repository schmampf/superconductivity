...
class ProjectCache:
    ...
    name: str
    path: str | Path = field(default_factory=Path.cwd)
...
def project_cache_path(path: str | Path, name: str) -> Path:
    ...
    return Path(path) / ".cache" / f"{safe_name}.pkl"


def make_cache(name: str, path: str | Path | None = None) -> ProjectCache:
    """Create an empty project cache."""
    return ProjectCache(name=name, path=Path.cwd() if path is None else path)
...
def _resolve_cache_file(
    *,
    ...
    if path is not None:
        return project_cache_path(path, str(name_or_path))
    if raw.suffix == ".pkl" or raw.parent != Path("."):
        return raw
    return project_cache_path(Path.cwd(), str(name_or_path))
