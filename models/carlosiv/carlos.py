...
def get_I_Delta_gauss(
    ...
    """
    Same as get_I_Delta, but uses Gauss–Legendre quadrature (_zint_gauss)
    with n_w nodes.

    This is a good candidate for production use: it is simpler than the
    adaptive integrator and tends to converge faster and more smoothly
    than fixed-grid Simpson.
    """
...
def get_I_Delta_fixed(
    ...
    """
    Same as get_I_Delta, but uses the fixed-grid composite Simpson integrator
    `_zint_fixed` with `n_w` grid points.

    Use this to benchmark accuracy vs the adaptive integrator `_zint` and to
    tune `n_w` for your desired precision/speed trade-off.
    """
    V_Delta = np.asarray(V_Delta, dtype=np.float64)
    tau = float(tau)
    temp = float(T_Delta)
...
def _zint_gauss(
    ...
    """
    Gauss–Legendre quadrature for _zintegrand(w).

    Uses n_w Gauss–Legendre nodes on [w_min, w_max]. This is usually more
    accurate than composite Simpson for the same number of points and often
    produces smoother error behaviour as a function of external parameters
    like V.

    Parameters
    ----------
    w_min, w_max :
        Integration limits in units of Delta.
    v, temp, thop, nan, eta :
        Parameters forwarded to _zintegrand.
    n_w :
        Number of Gauss–Legendre points (typically 32–128 is plenty).
    ...
    # Nodes x_i, weights w_i on [-1, 1]
    x_nodes, w_weights = leggauss(n_w)

    # Affine map to [w_min, w_max]
    mid = 0.5 * (w_max + w_min)
    half_range = 0.5 * (w_max - w_min)
    w_grid = mid + half_range * x_nodes  # shape (n_w,)

    # Evaluate integrand
    f_vals = np.empty(n_w, dtype=np.complex128)
    for i, wi in enumerate(w_grid):
        f_vals[i] = _zintegrand(
            w=wi,
            v=v,
            temp=temp,
            thop=thop,
    ...
    # Integral = sum_i w_i f(w_i) * (dw/dx) = half_range * sum_i w_i f(w_i)
    zint_val = half_range * np.sum(w_weights * f_vals)
    return zint_val
...
def _zint_fixed(
    ...
    """
    Fixed-grid composite Simpson integration of _zintegrand(w).

    This is meant as a faster, vectorizable alternative to the adaptive
    Simpson `_zint`. It uses a uniform grid with `n_w` points between
    `w_min` and `w_max` and applies the standard composite Simpson rule.
    ...
    v, temp, thop, nan, eta :
        Parameters forwarded to _zintegrand.
    n_w :
        Number of grid points (must be odd for Simpson). If even, it will
        be incremented by 1 internally.
    ...
    zint_val :
        Complex integral of _zintegrand(w) from w_min to w_max.
    """
    # Simpson requires an odd number of points (even number of sub-intervals)
    if n_w < 3:
        n_w = 3
    if n_w % 2 == 0:
        n_w += 1
    ...
    h = (w_max - w_min) / (n_w - 1)
    weights = np.ones(n_w, dtype=np.float64)
    # interior odd indices get weight 4, even indices get weight 2
    weights[1:-1:2] = 4.0
    weights[2:-2:2] = 2.0

    zint_val = (h / 3.0) * np.sum(weights * f_vals)
    return zint_val
