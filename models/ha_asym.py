...
def run_multiple_ha(
    ...
    with ThreadPoolExecutor(max_workers=n_worker) as executor:
        ...
        all_V_mV = np.array([], dtype="float64")
        all_I_nA = np.array([], dtype="float64")

        for future in as_completed(futures):
            result = future.result()
            all_V_mV = np.concatenate((all_V_mV, result[:, 0]))
            all_I_nA = np.concatenate((all_I_nA, result[:, 1]))
